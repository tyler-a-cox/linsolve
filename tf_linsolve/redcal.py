...
def add_pol_reds(reds, pols=["nn"], pol_mode="1pol"):
    ...
    for pol in pols:
        if pol_mode != "4pol_minV" or pol[0] == pol[1]:
            ...
            redsWithPols += [
                ([bl + (pol,) for bl in bls] + [bl + (conj_pol(pol),) for bl in bls])
                for bls in reds
            ]
...
def get_reds(
    antpos, pols=["nn"], pol_mode="1pol", bl_error_tol=1.0, include_autos=False
):
    ...
    """
    pos_reds = get_pos_reds(
        antpos, bl_error_tol=bl_error_tol, include_autos=include_autos
    )
...
def filter_reds(
    ...
    if min_bl_cut is not None or max_bl_cut is not None:
        ...
        lengths = [
            np.mean([np.linalg.norm(antpos[bl[1]] - antpos[bl[0]]) for bl in gp])
            for gp in reds
        ]
        reds = [
            gp
            ...
            if (
                (min_bl_cut is None or l > min_bl_cut)
    ...
    if max_dims is not None:
        while True:
            ...
            for dim, coords in enumerate(np.array(list(idealized_antpos.values())).T):
                rounded_coords = coords.round(
                    decimals=int(np.floor(-np.log10(IDEALIZED_BL_TOL)))
                )
            ...
            new_ex_ants = [
                ...
                if np.abs(idealized_antpos[ant][mode_dim] - mode_value)
                > IDEALIZED_BL_TOL
            ]
...
def reds_to_antpos(reds, tol=1e-10):
    ...
    antpos = {
        ant: np.array([1.0 if d + 1 == i else 0.0 for d in range(len(ants) - 1)])
        for i, ant in enumerate(ants)
    }
    for red in reds:
        for bl in red:
            # look for vectors in the higher dimensional space that are equal to 0
            delta = (antpos[bl[1]] - antpos[bl[0]]) - (
                antpos[red[0][1]] - antpos[red[0][0]]
            )
            if (
                np.linalg.norm(delta) > tol
            ):  # this baseline can help us reduce the dimensionality
                dim_to_elim = np.max(np.arange(len(delta))[np.abs(delta) > tol])
                antpos = {
                    ant: np.delete(
                        pos - pos[dim_to_elim] / delta[dim_to_elim] * delta, dim_to_elim
                    )
                    for ant, pos in antpos.items()
                }
    # remove any all-zero dimensions
    dim_to_elim = np.argwhere(
        np.sum(np.abs(list(antpos.values())), axis=0) == 0
    ).flatten()
...
def _build_polarity_baseline_groups(
    dly_cal_data, reds, edge_cut=0, max_rel_angle=(np.pi / 8)
):
    ...
    for red in reds:
        ...
        conj_median_bl = np.conj(
            np.median([dly_cal_data[bl][:, fslice] for bl in red], axis=0)
        )
        for bl in red:
            # compare the median baseline to this baseline, taking the median abs of the angle over time and freq
            median_abs_relative_angle = np.median(
                np.abs(np.angle(conj_median_bl * dly_cal_data[bl][:, fslice]))
            )
...
def _infer_polarity_flips(polarity_groups, prior_is_flipped, prior_even_vs_odd_IDs):
    ...
    """
    is_flipped, even_vs_odd_IDs = (
        deepcopy(prior_is_flipped),
        deepcopy(prior_even_vs_odd_IDs),
    )
    ...
    while True:
        ...
        for key, (grp1, grp2) in polarity_groups.items():
            ...
            if key in even_vs_odd_IDs:
                even, odd = {"even/odd": (grp1, grp2), "odd/even": (grp2, grp1)}[
                    even_vs_odd_IDs[key]
                ]
            ...
            # try to infer if this group is even/odd or odd/even from two known flips
                ...
                for labels, grp in zip(
                    [["even/odd", "odd/even"], ["odd/even", "even/odd"]], [grp1, grp2]
                ):
                    for bl in grp:
                        ant0, ant1 = utils.split_bl(bl)
                        if (ant0 in is_flipped) and (ant1 in is_flipped):
...
def _recursive_try_assumptions(
    polarity_groups,
    ants,
    prior_is_flipped,
    prior_even_vs_odd_IDs,
    depth,
    ...
    if len(prior_is_flipped) == len(ants):
        _check_polarity_results(
            polarity_groups, prior_is_flipped, prior_even_vs_odd_IDs
        )
    ...
    group_keys = sorted(
        polarity_groups,
        key=lambda k: len(polarity_groups[k][0]) - len(polarity_groups[k][1]),
        reverse=True,
    )
    ...
    for assumed_ID in ["even/odd", "odd/even"]:
        ...
        prior_is_flipped = _find_starting_is_flipped(
            polarity_groups, ants, even_vs_odd_IDs
        )
        try:
            new_is_flipped, new_even_vs_odd_IDs = _infer_polarity_flips(
                polarity_groups, prior_is_flipped, even_vs_odd_IDs
            )
            return _recursive_try_assumptions(
                polarity_groups,
                ants,
                new_is_flipped,
                new_even_vs_odd_IDs,
...
def find_polarity_flipped_ants(
    dly_cal_data, reds, edge_cut=0, max_rel_angle=(np.pi / 8), max_recursion_depth=6
):
    ...
    polarity_groups = _build_polarity_baseline_groups(
        dly_cal_data, reds, edge_cut=edge_cut, max_rel_angle=np.pi / 8
    )

    try:
        is_flipped, even_vs_odd_IDs = _recursive_try_assumptions(
            polarity_groups, ants, {}, {}, 1, max_recursion_depth=5
        )
...
def _check_polLists_minV(polLists):
    ...
    for polList in polLists:
        ...
        if len(polList) == 1:
            ...
            if (
                polList[0] != conj_pol(polList[1])
                or split_pol(polList[0])[0] == split_pol(polList[0])[1]
            ):
                return False
        else:
            return False
...
def parse_pol_mode(reds):
    ...
    if len(pols) == 1 and len(antpols) == 1:
        return "1pol"
    elif len(pols) == 2 and np.all(
        [split_pol(pol)[0] == split_pol(pol)[1] for pol in pols]
    ):
        return "2pol"
...
class OmnicalSolver(linsolve.LinProductSolver):
    ...
    def solve_iteratively(
        self, conv_crit=1e-10, maxiter=50, check_every=4, check_after=1, verbose=False
    ):
        ...
        chisq = sum(
            [np.abs(self.data[k] - dmdl_u[k]) ** 2 * self.wgts[k] for k in self.keys]
        )
        ...
        for i in range(1, maxiter + 1):
            ...
            if (i % check_every) == 1:
                ...
                dwgts_u = {
                    k: dmdl_u[k] * dmdl_u[k].conj() * wgts_u[k] for k in self.keys
                }
            ...
            if i < maxiter and (i < check_after or (i % check_every) != 0):
                ...
                new_chisq_u = sum(
                    [
                        np.abs(v[update] - dmdl_u[k]) ** 2 * wgts_u[k]
...
class RedundantCalibrator:
    def __init__(self, reds, check_redundancy=False):
        ...
        if check_redundancy:
            ...
            if nDegens != nDegensExpected:
                ...
                raise ValueError(
                    "{} degeneracies found, but {} ".format(nDegens, nDegensExpected)
                    + "degeneracies expected (assuming {} phase slopes).".format(
                        nPhaseSlopes
                    )
                )
        ...
        for ubl_index, blgrp in enumerate(self.reds):
            for ant_i, ant_j, pol in blgrp:
                if dc is not None and (ant_i, ant_j, pol) not in dc:
                    raise KeyError(
                        "Baseline {} not in provided DataContainer".format(
                            (ant_i, ant_j, pol)
                        )
                    )
                params = (
    ...
    def _solver(self, solver, data, wgts={}, detrend_phs=False, **kwargs):
        ...
        if detrend_phs:
            for blgrp in self.reds:
                self.phs_avg[blgrp[0]] = np.exp(
                    -np.complex64(1j)
                    * np.median(
                        np.unwrap([np.log(dc[bl]).imag for bl in blgrp], axis=0), axis=0
                    )
                )
                for bl in blgrp:
    ...
    def compute_ubls(self, data, gain_sols):
        ...
        for ubl, blgrp in enumerate(self.reds):
            ...
            ubl_sols[blgrp[0]] = np.average(
                d_gp, axis=0
            )  # XXX add option for median here?
        return ubl_sols

    def _firstcal_iteration(
        self,
        data,
        df,
        f0,
        ...
        dly_sol = {
            ant: dly_sol.get(ant, (np.zeros_like(list(dly_sol.values())[0])))
            for ant in ants
        }
        off_sol = {
            ant: off_sol.get(ant, (np.zeros_like(list(off_sol.values())[0])))
            for ant in ants
        }
    ...
    def firstcal(
        ...
        for i in range(maxiter):
            dlys, delta_off = self._firstcal_iteration(
                data,
                df=df,
                f0=freqs[0],
                wgts=wgts,
            ...
            if (
                i == 0
            ):  # only solve for delays on the first iteration, also apply polarity flips
                g_fc = {
                    ant: np.array(
                        np.exp(2j * np.pi * np.outer(dly, freqs)), dtype=dtype
                    )
                    for ant, dly in dlys.items()
                }
                calibrate_in_place(
                    data, g_fc, gain_convention="divide"
                )  # applies calibration
                ...
                polarity_flips = find_polarity_flipped_ants(
                    data,
                    self.reds,
                ...
                meta["polarity_flips"] = {
                    ant: np.array([polarity_flips[ant] for i in range(len(dlys[ant]))])
                    for ant in polarity_flips
                }
                if np.all([flip is not None for flip in polarity_flips.values()]):
                    polarities = {
                        ant: -1.0 if polarity_flips[ant] else 1.0 for ant in g_fc
                    }
                    calibrate_in_place(
                        data, polarities, gain_convention="divide"
                    )  # applies calibration
                ...
                delta_gains = {
                    ant: np.array(
                        np.ones_like(g_fc[ant]) * np.exp(1.0j * delta_off[ant]),
                        dtype=dtype,
                    )
                    for ant in g_fc.keys()
                }
                calibrate_in_place(
                    data, delta_gains, gain_convention="divide"
                )  # update calibration
        ...
        calibrate_in_place(
            data, g_fc, gain_convention="multiply"
        )  # unapply calibration
        return meta, g_fc
    ...
    def logcal(self, data, sol0={}, wgts={}, sparse=False, mode="default"):
        ...
        sol_with_fc = {
            key: (sol[key] * sol0[key] if (key in sol0 and len(key) == 2) else sol[key])
            for key in sol.keys()
        }
        return {}, sol_with_fc
    ...
    def remove_degen_gains(self, gains, degen_gains=None, mode="phase"):
        ...
        if self.pol_mode == "2pol":
            ...
            new_gains = self.remove_degen_gains(
                pol0_gains, degen_gains=degen_gains, mode=mode
            )
            new_gains.update(
                self.remove_degen_gains(pol1_gains, degen_gains=degen_gains, mode=mode)
            )
            self.pol_mode = "2pol"
            return new_gains
        ...
        if mode == "phase":
            ...
            # Fix phase terms
            degenToRemove = np.einsum(
                "ij,jkl", Mgains, np.angle(gainSols * np.conj(degenGains))
            )
            gainSols *= np.exp(
                np.complex64(-1j) * np.einsum("ij,jkl", Rgains, degenToRemove)
            )
            ...
            for pol in antpols:
                meanSqAmplitude = np.mean(
                    [
                        np.abs(g1 * g2)
                        for (a1, p1), g1 in gains.items()
                        for (a2, p2), g2 in gains.items()
                        if p1 == pol and p2 == pol and a1 != a2
                    ],
                    axis=0,
                )
                degenMeanSqAmplitude = np.mean(
                    [
                        np.abs(degen_gains[k1] * degen_gains[k2])
                        for k1 in gains.keys()
                        for k2 in gains.keys()
                        if k1[1] == pol and k2[1] == pol and k1[0] != k2[0]
                    ],
                    axis=0,
                )
                gainSols[gainPols == pol] *= (
                    degenMeanSqAmplitude / meanSqAmplitude
                ) ** 0.5
    ...
    def remove_degen(self, sol, degen_sol=None):
        ...
        new_gains = self.remove_degen_gains(
            gains, degen_gains=degen_sol, mode="complex"
        )
    ...
    def count_degens(self, assume_redundant=True):
        ...
        if assume_redundant:
            ...
            dummy_data = DataContainer(
                {
                    bl: np.ones((1, 1), dtype=np.complex)
...
def is_redundantly_calibratable(antpos, bl_error_tol=1.0, require_coplanarity=True):
    ...
    if require_coplanarity:
        if (
            len(list(reds_to_antpos(reds).values())[0]) > 2
        ):  # not a monolithic, coplanar array
            return False
...
def predict_chisq_per_ant(reds):
    ...
    return {
        ant: np.sum([predicted_chisq_per_bl[bl] for bl in bls if ant in split_bl(bl)])
        for ant in ants
    }
...
def normalized_chisq(data, data_wgts, reds, vis_sols, gains):
    ...
    chisq, _, chisq_per_ant, _ = utils.chisq(
        data,
        vis_sols,
        data_wgts=data_wgts,
        gains=gains,
        reds=reds,
        split_by_antpol=(pol_mode in ["1pol", "2pol"]),
    )
    predicted_chisq_per_ant = predict_chisq_per_ant(reds)
    chisq_per_ant = {
        ant: cs / predicted_chisq_per_ant[ant] for ant, cs in chisq_per_ant.items()
    }
    if pol_mode in ["1pol", "2pol"]:  # in this case, chisq is split by antpol
        for antpol in chisq.keys():
            chisq[antpol] /= np.sum(
                [
                    cspa / 2.0
                    for ant, cspa in predicted_chisq_per_ant.items()
                    if antpol in ant
                ],
                axis=0,
            )
...
def _get_pol_load_list(pols, pol_mode="1pol"):
    ...
    if pol_mode in ["1pol", "2pol"]:
        pol_load_list = [
            [pol] for pol in pols if split_pol(pol)[0] == split_pol(pol)[1]
        ]
    elif pol_mode in ["4pol", "4pol_minV"]:
        assert (
            len(pols) == 4
        ), "For 4pol calibration, there must be four polarizations in the data file."
        pol_load_list = [pols]
    else:
...
def linear_cal_update(
    bls, cal, data, all_reds, weight_by_nsamples=False, weight_by_flags=False
):
    ...
    consts = {
        rc_all.pack_sol_key(ant): cal["g_omnical"][ant] for ant in cal["g_omnical"]
    }
    ...
    for bl in bls:
        ...
        if weight_by_nsamples:
            bl_wgts[bl] *= cal["vns_omnical"][
                bl_to_ubl_map[bl]
            ]  # weight by nsamples in the bl group
        if weight_by_flags:
...
def expand_omni_sol(cal, all_reds, data, nsamples):
    ...
    dts_by_bl = DataContainer(
        {
            bl: np.median(np.ediff1d(data.times_by_bl[bl[:2]])) * SEC_PER_DAY
            for bl in good_ants_bls
        }
    )
    data_wgts = DataContainer(
        {
            bl: predict_noise_variance_from_autos(bl, data, dt=dts_by_bl[bl]) ** -1
            for bl in good_ants_bls
        }
    )
    cal["chisq"], cal["chisq_per_ant"] = normalized_chisq(
        data, data_wgts, good_ants_reds, cal["v_omnical"], cal["g_omnical"]
    )
    ...
    for red in all_reds:
        cal["vns_omnical"][red[0]] = np.sum(
            [
                nsamples[bl]
                * (
                    (split_bl(bl)[0] in cal["g_omnical"])
    ...
    for i in range(len(data)):  # this should break well before the end
        ...
        for ant in new_gain_ants:
            new_gains.update(
                linear_cal_update(
                    [bl for bl in bls_to_use if ant in split_bl(bl)],
                    cal,
                    data,
                    all_reds,
        ...
        _, _, chisq_per_ant, _ = utils.chisq(
            data_subset,
            cal["v_omnical"],
            data_wgts=data_wgts,
            gains=cal["g_omnical"],
        ...
        for ant, cspa in chisq_per_ant.items():
            if ant not in cal["chisq_per_ant"]:
                cal["chisq_per_ant"][ant] = (
                    chisq_per_ant[ant] / predicted_chisq_per_ant[ant]
                )
                cal["chisq_per_ant"][ant][~np.isfinite(cspa)] = np.zeros_like(
                    cspa[~np.isfinite(cspa)]
                )
    ...
    for red in all_reds:
        ...
        red_to_solve_for = []
        for bl in red:
            if (split_bl(bl)[0] in cal["g_omnical"]) and (
                split_bl(bl)[1] in cal["g_omnical"]
            ):
...
def redundantly_calibrate(
    ...
    # perform firstcal
    rv["fc_meta"], rv["g_firstcal"] = rc.firstcal(
        data, freqs, maxiter=fc_maxiter, conv_crit=fc_conv_crit
    )
    rv["gf_firstcal"] = {
        ant: np.zeros_like(g, dtype=bool) for ant, g in rv["g_firstcal"].items()
    }
    ...
    data_wgts = {
        bl: predict_noise_variance_from_autos(
            bl, data, dt=(np.median(np.ediff1d(times_by_bl[bl[:2]])) * SEC_PER_DAY)
        )
        ** -1
        for bl in data.keys()
    }
    ...
    rv["vf_omnical"] = DataContainer(
        {bl: ~np.isfinite(v) for bl, v in rv["v_omnical"].items()}
    )
    ...
    rv["g_omnical"] = {
        ant: g * ~rv["gf_omnical"][ant] + rv["gf_omnical"][ant]
        for ant, g in rv["g_omnical"].items()
    }

    # compute chisqs
    rv["chisq"], rv["chisq_per_ant"] = normalized_chisq(
        data, data_wgts, filtered_reds, rv["v_omnical"], rv["g_omnical"]
    )
    return rv


def redcal_iteration(
    hd,
    nInt_to_load=None,
    pol_mode="2pol",
    bl_error_tol=1.0,
    ...
    **filter_reds_kwargs
    ...
    if nInt_to_load is not None:
        assert (
            hd.filetype == "uvh5"
        ), "Partial loading only available for uvh5 filetype."
    else:
        if (
            hd.data_array is None
        ):  # if data loading hasn't happened yet, load the whole file
            hd.read()
    ...
    rv = {}  # dictionary of return values
    rv["g_firstcal"] = {
        ant: np.ones((nTimes, nFreqs), dtype=np.complex64) for ant in ants
    }
    ...
    rv["chisq"] = {
        antpol: np.zeros((nTimes, nFreqs), dtype=np.float32) for antpol in antpols
    }
    rv["chisq_per_ant"] = {
        ant: np.zeros((nTimes, nFreqs), dtype=np.float32) for ant in ants
    }

    # get reds and then intitialize omnical visibility solutions to all 1s and all flagged
    all_reds = get_reds(
        {ant: hd.antpos[ant] for ant in ant_nums},
        bl_error_tol=bl_error_tol,
        pol_mode=pol_mode,
        pols=set([pol for pols in pol_load_list for pol in pols]),
    )
    rv["v_omnical"] = DataContainer(
        {red[0]: np.ones((nTimes, nFreqs), dtype=np.complex64) for red in all_reds}
    )
    rv["vf_omnical"] = DataContainer(
        {red[0]: np.ones((nTimes, nFreqs), dtype=bool) for red in all_reds}
    )
    rv["vns_omnical"] = DataContainer(
        {red[0]: np.zeros((nTimes, nFreqs), dtype=np.float32) for red in all_reds}
    )
    filtered_reds = filter_reds(
        all_reds, ex_ants=ex_ants, antpos=hd.antpos, **filter_reds_kwargs
    )
    ...
    rv["omni_meta"] = {
        "chisq": {
            str(pols): np.zeros((nTimes, nFreqs), dtype=float) for pols in pol_load_list
        }
    }
    rv["omni_meta"]["iter"] = {
        str(pols): np.zeros((nTimes, nFreqs), dtype=int) for pols in pol_load_list
    }
    rv["omni_meta"]["conv_crit"] = {
        str(pols): np.zeros((nTimes, nFreqs), dtype=float) for pols in pol_load_list
    }
    ...
    if verbose and np.any(solar_flagged):
        print(
            len(hd.times[solar_flagged]),
            "integrations flagged due to sun above",
            solar_horizon,
            "degrees.",
        )
            ...
            if len(tinds) > 0:
                ...
                if nInt_to_load is None:  # don't perform partial I/O
                    (
                        ...
                        hd.build_datacontainers()
                    )  # this may contain unused polarizations, but that's OK
                    for bl in data:
                        data[bl] = data[bl][
                            tinds, fSlice
                        ]  # cut down size of DataContainers to match unflagged indices
                        nsamples[bl] = nsamples[bl][tinds, fSlice]
                else:  # perform partial i/o
                    data, _, nsamples = hd.read(
                        times=hd.times[tinds],
                        frequencies=hd.freqs[fSlice],
                        polarizations=pols,
                    )
                cal = redundantly_calibrate(
                    data,
                    reds,
                    freqs=hd.freqs[fSlice],
                    times_by_bl=hd.times_by_bl,
                    fc_conv_crit=fc_conv_crit,
                    fc_maxiter=fc_maxiter,
                    oc_conv_crit=oc_conv_crit,
                ...
                for ant in cal["fc_meta"]["dlys"].keys():
                    ...
                    rv["fc_meta"]["polarity_flips"][ant][tinds] = cal["fc_meta"][
                        "polarity_flips"
                    ][ant]
                ...
                rv["omni_meta"]["chisq"][str(pols)][tinds, fSlice] = cal["omni_meta"][
                    "chisq"
                ]
                rv["omni_meta"]["iter"][str(pols)][tinds, fSlice] = cal["omni_meta"][
                    "iter"
                ]
                rv["omni_meta"]["conv_crit"][str(pols)][tinds, fSlice] = cal[
                    "omni_meta"
                ]["conv_crit"]
...
def _redcal_run_write_results(
    ...
    antenna_positions = np.array(
        [
            hd.antenna_positions[hd.antenna_numbers == antnum].flatten()
            for antnum in cal_antnums
        ]
    )
    ...
    write_cal(
        ...
        hd.freqs,
        hd.times,
        flags=cal["gf_firstcal"],
        outdir=outdir,
        overwrite=clobber,
        x_orientation=hd.x_orientation,
        telescope_location=hd.telescope_location,
    ...
    write_cal(
        ...
        hd.freqs,
        hd.times,
        flags=cal["gf_omnical"],
        quality=cal["chisq_per_ant"],
        total_qual=cal["chisq"],
        outdir=outdir,
        overwrite=clobber,
        x_orientation=hd.x_orientation,
        telescope_location=hd.telescope_location,
    ...
    if verbose:
        print(
            "Now saving omnical visibilities to", os.path.join(outdir, omnivis_filename)
        )
    hd_out = HERAData(hd.filepaths[0], filetype=hd.filetype)
    hd_out.read(bls=cal["v_omnical"].keys())
    hd_out.update(
        data=cal["v_omnical"], flags=cal["vf_omnical"], nsamples=cal["vns_omnical"]
    )
    ...
    save_redcal_meta(
        os.path.join(outdir, meta_filename),
        cal["fc_meta"],
        cal["omni_meta"],
        hd.freqs,
        hd.times,
...
def redcal_run(
    ...
    **filter_reds_kwargs
    ...
    if isinstance(input_data, str):
        ...
        raise TypeError(
            "input_data must be a single string path to a visibility data file or a HERAData object"
        )
    ...
    if a_priori_ex_ants_yaml is not None:
        ...
        ex_ants = ex_ants.union(
            set(read_a_priori_ant_flags(a_priori_ex_ants_yaml, ant_indices_only=True))
        )
    high_z_ant_hist = ""
    ...
    while True:
        ...
        z_scores = per_antenna_modified_z_scores(
            {
                ant: np.nanmedian(cspa)
        ...
        if run_number == 1 and len(iter0_prefix) > 0:
            _redcal_run_write_results(
                cal,
                hd,
                filename_no_ext + iter0_prefix + firstcal_ext,
                filename_no_ext + iter0_prefix + omnical_ext,
                filename_no_ext + iter0_prefix + omnivis_ext,
                filename_no_ext + iter0_prefix + meta_ext,
                outdir,
                clobber=clobber,
                verbose=verbose,
    ...
    _redcal_run_write_results(
        cal,
        hd,
        filename_no_ext + firstcal_ext,
        filename_no_ext + omnical_ext,
        filename_no_ext + omnivis_ext,
        filename_no_ext + meta_ext,
        outdir,
        clobber=clobber,
        verbose=verbose,
        add_to_history=add_to_history + "\n" + high_z_ant_hist,
...
def redcal_argparser():
    ...
    a = argparse.ArgumentParser(
        description="Redundantly calibrate a file using hera_cal.redcal. This includes firstcal, logcal, and omnical. \
                                Iteratively re-runs by flagging antennas with large chi^2. Saves the result to calfits and uvh5 files."
    )
    a.add_argument(
        "input_data", type=str, help="path to uvh5 visibility data file to calibrate."
    )
    a.add_argument(
        "--firstcal_ext",
        default=".first.calfits",
        type=str,
        help="string to replace file extension of input_data for saving firstcal calfits",
    )
    a.add_argument(
        "--omnical_ext",
        default=".omni.calfits",
        type=str,
        help="string to replace file extension of input_data for saving omnical calfits",
    )
    a.add_argument(
        "--omnivis_ext",
        default=".omni_vis.uvh5",
        type=str,
        help="string to replace file extension of input_data for saving omnical visibilities as uvh5",
    )
    a.add_argument(
        "--meta_ext",
        default=".redcal_meta.hdf5",
        type=str,
        help="string to replace file extension of input_data for saving metadata as hdf5",
    )
    a.add_argument(
        "--outdir",
        default=None,
        type=str,
        help="folder to save data products. Default is the same as the folder containing input_data",
    )
    a.add_argument(
        "--iter0_prefix",
        default="",
        type=str,
        help="if not default '', save the omnical results with this prefix appended to each file after the 0th iteration, \
    ...
    a.add_argument(
        ...
        default=False,
        action="store_true",
        help="overwrites existing files for the firstcal and omnical results",
    )
    a.add_argument(
        "--verbose",
        default=False,
        action="store_true",
        help="print calibration progress updates",
    )

    redcal_opts = a.add_argument_group(title="Runtime Options for Redcal")
    redcal_opts.add_argument(
        "--metrics_files",
        type=str,
        nargs="*",
        default=[],
        help="path to file containing ant_metrics or auto_metrics readable by hera_qm.metrics_io.load_metric_file. \
    ...
    redcal_opts.add_argument(
        ...
        nargs="*",
        default=[],
        help="space-delimited list of antennas to exclude from calibration and flag. All pols for an antenna will be excluded.",
    )
    redcal_opts.add_argument(
        "--a_priori_ex_ants_yaml",
        type=str,
        default=None,
        help="path to YAML file containing a priori ex_ants parsable by hera_qm.metrics_io.read_a_priori_ant_flags()",
    )
    redcal_opts.add_argument(
        "--ant_z_thresh",
        type=float,
        default=4.0,
        help="Threshold of modified z-score for chi^2 per antenna above which antennas are thrown away and calibration is re-run iteratively.",
    )
    redcal_opts.add_argument(
        "--max_rerun",
        type=int,
        default=5,
        help="Maximum number of times to re-run redundant calibration.",
    )
    redcal_opts.add_argument(
        "--solar_horizon",
        type=float,
        default=0.0,
        help="When the Sun is above this altitude in degrees, calibration is skipped and the integrations are flagged.",
    )
    redcal_opts.add_argument(
        "--flag_nchan_low",
        type=int,
        default=0,
        help="integer number of channels at the low frequency end of the band to always flag (default 0)",
    )
    redcal_opts.add_argument(
        "--flag_nchan_high",
        type=int,
    ...
    redcal_opts.add_argument(
        ...
        type=int,
        default=None,
        help="number of integrations to load and calibrate simultaneously. Lower numbers save memory, but incur a CPU overhead. \
    ...
    redcal_opts.add_argument(
        ...
        type=str,
        default="2pol",
        help="polarization mode of redundancies. Can be '1pol', '2pol', '4pol', or '4pol_minV'. See recal.get_reds documentation.",
    )
    redcal_opts.add_argument(
        "--bl_error_tol",
        type=float,
        default=1.0,
        help="the largest allowable difference between baselines in a redundant group",
    )
    redcal_opts.add_argument(
        "--min_bl_cut",
        type=float,
        default=None,
        help="cut redundant groups with average baseline lengths shorter than this length in meters",
    )
    redcal_opts.add_argument(
        "--max_bl_cut",
        type=float,
        default=None,
        help="cut redundant groups with average baseline lengths longer than this length in meters",
    )
    redcal_opts.add_argument(
        "--max_dims",
        type=float,
        default=2,
        help='maximum allowed tip/tilt phase degeneracies of redcal. None is no limit. Default 2 is a classically \
    ...
    omni_opts.add_argument(
        "--fc_conv_crit",
        type=float,
        default=1e-6,
        help="maximum allowed changed in firstcal phases for convergence",
    )
    omni_opts.add_argument(
        "--fc_maxiter",
        type=int,
        default=50,
        help="maximum number of firstcal iterations allowed for finding per-antenna phases",
    )
    omni_opts.add_argument(
        "--oc_conv_crit",
        type=float,
        default=1e-10,
        help="maximum allowed relative change in omnical solutions for convergence",
    )
    omni_opts.add_argument(
        "--oc_maxiter",
        type=int,
        default=500,
        help="maximum number of omnical iterations allowed before it gives up",
    )
    omni_opts.add_argument(
        "--check_every",
        type=int,
        default=10,
        help="compute omnical convergence every Nth iteration (saves computation).",
    )
    omni_opts.add_argument(
        "--check_after",
        type=int,
        default=50,
        help="start computing omnical convergence only after N iterations (saves computation).",
    )
    omni_opts.add_argument(
        "--gain",
        type=float,
        default=0.4,
        help="The fractional step made toward the new solution each omnical iteration. Values in the range 0.1 to 0.5 are generally safe.",
    )
